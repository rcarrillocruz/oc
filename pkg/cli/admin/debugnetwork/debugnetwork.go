...
var (
	debugNetworkLong = templates.LongDesc(`
		Launch a pod to debug networking
	...
	debugNetworkExample = templates.Examples(`
		# debug network using the default plug-in image and command, writing into ./debug-network.local.<rand>
		  oc adm debug-network

		# debug network with a specific local folder to copy to
		  oc adm debug-network --dest-dir=/local/directory

		# debug network audit information
		  oc adm debug-network -- /usr/bin/gather_audit_logs

		# debug network using multiple plug-in images
		  oc adm debug-network --image=quay.io/kubevirt/debug-network --image=quay.io/openshift/origin-debug-network

		# debug network using a specific image stream plug-in
		  oc adm debug-network --image-stream=openshift/debug-network:latest

		# debug network using a specific image and command
		  oc adm debug-network --image=my/image:tag -- myspecial-command.sh
	`)
)

func NewDebugNetworkCommand(f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	o := NewDebugNetworkOptions(streams)
	cmd := &cobra.Command{
		Use:     "debug-network",
		Short:   "Launch a new instance of a pod for debugging networking",
		Long:    debugNetworkLong,
		Example: debugNetworkExample,
	...
	cmd.Flags().StringSliceVar(&o.Images, "image", o.Images, "Specify a debug-network plugin image to run. If not specified, OpenShift's default debug-network image will be used.")
	cmd.Flags().StringSliceVar(&o.ImageStreams, "image-stream", o.ImageStreams, "Specify an image stream (namespace/name:tag) containing a debug-network plugin image to run.")
	...
	cmd.Flags().Int64Var(&o.Timeout, "timeout", 600, "The length of time to debug network, in seconds. Defaults to 10 minutes.")
...
func NewDebugNetworkOptions(streams genericclioptions.IOStreams) *DebugNetworkOptions {
	return &DebugNetworkOptions{
		SourceDir: "/debug-network/",
		IOStreams: streams,
		LogOut:    newPrefixWriter(streams.Out, "[debug-network      ] OUT"),
	}
}

func (o *DebugNetworkOptions) Complete(f kcmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	if len(o.DestDir) == 0 {
		o.DestDir = fmt.Sprintf("debug-network.local.%06d", rand.Int63())
	}
...
func (o *DebugNetworkOptions) completeImages() error {
	...
	if len(o.Images) == 0 {
		var image string
		var err error
		if image, err = o.resolveImageStreamTag("openshift", "debug-network", "latest"); err != nil {
			o.log("%v\n", err)
			image = "quay.io/openshift/origin-debug-network:latest"
		}
		o.Images = append(o.Images, image)
	}
	o.log("Using debug-network plug-in image: %s", strings.Join(o.Images, ", "))
	return nil
}

func (o *DebugNetworkOptions) resolveImageStreamTagString(s string) (string, error) {
...
func (o *DebugNetworkOptions) resolveImageStreamTag(namespace, name, tag string) (string, error) {
...
type DebugNetworkOptions struct {
...
func (o *DebugNetworkOptions) Validate() error {
	if len(o.Images) == 0 {
...
// Run creates and runs a debug-network pod.d
func (o *DebugNetworkOptions) Run() error {
	var err error

	// create namespace ...
	ns, err := o.Client.CoreV1().Namespaces().Create(context.TODO(), &corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			GenerateName: "openshift-debug-network-",
			Labels: map[string]string{
				"openshift.io/run-level": "0",
			},
			Annotations: map[string]string{
				"oc.openshift.io/command":    "oc adm debug-network",
	...
	for _, pod := range pods {
		go func(pod *corev1.Pod) {
			...
			// wait for debug-network container to be running (gather is running)
			if err := o.waitForDebugNetworkContainerRunning(pod); err != nil {
				log("debug-network did not start: %s", err)
				errCh <- fmt.Errorf("debug-network did not start for pod %s: %s", pod.Name, err)
				return
			}
			// stream debug-network container logs
			if err := o.getDebugNetworkContainerLogs(pod); err != nil {
				log("debug-network logs unavailable: %v", err)
			}

			// wait for pod to be running (debug-network has completed)
			log("waiting for debug-network to complete")
			if err := o.waitForDebugNetworkToComplete(pod); err != nil {
				log("debug-network never finished: %v", err)
				errCh <- fmt.Errorf("debug-network never finished for pod %s: %s", pod.Name, err)
				return
			}
...
func (o *DebugNetworkOptions) log(format string, a ...interface{}) {
	fmt.Fprintf(o.LogOut, format+"\n", a...)
}

func (o *DebugNetworkOptions) logTimestamp() error {
...
func (o *DebugNetworkOptions) copyFilesFromPod(pod *corev1.Pod) error {
	streams := o.IOStreams
...
func (o *DebugNetworkOptions) getDebugNetworkContainerLogs(pod *corev1.Pod) error {
...
func (o *DebugNetworkOptions) waitForDebugNetworkToComplete(pod *corev1.Pod) error {
...
func (o *DebugNetworkOptions) waitForDebugNetworkContainerRunning(pod *corev1.Pod) error {
...
func (o *DebugNetworkOptions) newClusterRoleBinding(ns string) *rbacv1.ClusterRoleBinding {
	return &rbacv1.ClusterRoleBinding{
		ObjectMeta: metav1.ObjectMeta{
			GenerateName: "debug-network-",
			Annotations: map[string]string{
				"oc.openshift.io/command": "oc adm debug-network",
			},
		},
...
func (o *DebugNetworkOptions) newPod(node, image string) *corev1.Pod {
	zero := int64(0)
			...
			Containers: []corev1.Container{
				{
					...
					Command: []string{"/bin/bash", "-c", "/usr/bin/debug-network; sync"},
